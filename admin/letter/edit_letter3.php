<?php 
    ...
    $lid=$_GET['lid'];
    $letter3=mysqli_fetch_array(mysqli_query($conn,"SELECT * FROM letter_3 WHERE id='$lid'"));
    
    $letter3_items=mysqli_query($conn,"SELECT * FROM letter_three_items WHERE letter_id='$lid'");
    ...
    if (isset($_POST['submit'])) {
        ...
        $add_letter=mysqli_query($conn,"UPDATE letter_3 SET b_id='$bureau',datee='$date',note='$additional' WHERE id='$lid' ");
        if ($add_letter) {
            $delete=mysqli_query($conn,"DELETE FROM letter_three_items WHERE letter_id='$lid'");
             ...
             if (isset($_POST['item'])) {
                foreach($_POST['item'] as $key=>$val){
                    ...
                    $item=mysqli_query($conn,"INSERT INTO letter_three_items(letter_id,item,reason_id)VALUES ('$lid','$item','$letter_reason')");
                    if ($item==false) {
                        echo $conn->error;
                    }
                }
                if ($add_letter==true && $item==true) {
                     header("Location: letter_preview3.php?id=$lid");    
                                ...
                                <form action="" method="POST" >
                                    <div class="form-row mb-4">
                                        <div class="form-group col-md-6">
                                            <label>Date:</label>
                                            <input type="date" name="date" value="<?php echo $letter3['datee'] ?>" class="form-control" required> 
                                        </div>
                                        <div class="form-group col-md-6" >
                                            <label>Bureaus:</label>
                                        ...
                                        <div class="form-group col-md-12" id="body">
                                            <?php 
                                                $count=0;
                                                while ($items=mysqli_fetch_array($letter3_items)) {
                                                    $reasons=mysqli_query($conn,"SELECT * FROM reason_of_letter");
                                            ?>
                                        ...
                                        <div class="form-group col-md-12 mt-4" >
                                            ...
                                            <textarea id="editor" name="additional" rows="5"><?php echo $letter3['note']; ?></textarea>
                                        </div>
                                        <hr>
                                    </div>
