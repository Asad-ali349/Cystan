<?php 
    ...
    if (isset($_POST['submit'])) {
        ...
        $add_letter=mysqli_query($conn,"INSERT INTO `letter_2`(b_id,datee,req_id,note) VALUES('$bureau','$date','$srid','$additional')");
        if ($add_letter) {
        	$get_id=mysqli_fetch_array(mysqli_query($conn,"SELECT id from letter_2 WHERE `b_id`='$bureau' AND `datee`='$date' AND `req_id`='$srid' AND `note`='$additional'"));
                $lid=$get_id['id'];
