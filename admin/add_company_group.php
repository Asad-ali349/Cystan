...
$groups=mysqli_query($conn,"SELECT grouping.id,grouping.name,grouping.pin_color_id,grouping.category_id,pin_color.color_code FROM grouping INNER JOIN pin_color ON grouping.pin_color_id = pin_color.id WHERE category_id='1'");
if (isset($_POST['submit'])) {
    $name=$_POST['name'];
    $color=$_POST['color'];
    
    $addGroup=mysqli_query($conn,"INSERT INTO grouping(name,pin_color_id,category_id) VALUES('$name','$color','1')");
    $groups=mysqli_query($conn,"SELECT grouping.id,grouping.name,grouping.pin_color_id,grouping.category_id,pin_color.color_code FROM grouping INNER JOIN pin_color ON grouping.pin_color_id = pin_color.id WHERE category_id='1'");
    if($addGroup==TRUE) {
        $success="Group Added";
    }else{
                            ...
                            <table id="html5-extension" class="table table-hover non-hover" style="width:100%">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div style="width: 20px;height: 20px;border-radius: 20px ;background-color: <?php echo $data2['color_code']; ?>"></div>
                                        </td>
                                        <td>
                                            ...
                                            <a href="deleteGroup.php?id=<?php echo $data2['id'];?>&table=grouping&page=add_company_group.php" onclick="return confirm('Are you sure?')"><i class="far fa-times-circle" style="color: red; font-size: 20px;" aria-hidden="true"></i></a>
                                        </td>
                                          ...
                                          <div class="modal-dialog" role="document">
                                            <div class="modal-content">
                                              ...
                                              <div class="modal-body">
                                                    ...
                                                    <label>Name:</label>
                                                    <input type="text" class="form-control"  value="<?php echo $data2['name']; ?>" id="name<?php echo $data2['id'];?>">
                                                    <label>Pin Color:</label>
                                                    <select class="form-control" id="color<?php echo $data2['id'];?>" >
                                                    <option class="" value="">Select Pin Color</option>
                                                    <?php
                                                        $color=mysqli_query($conn,"SELECT * FROM pin_color");
                                                        while ($data3=mysqli_fetch_array($color)) {
                                                            if ($data2['pin_color_id']==$data3['id']) {
                                                    ?>
                                              ...
                                              <div class="modal-footer">
                                                    <button class="btn btn-danger" onclick="updateGroup(name<?php echo $data2['id'];?>,color<?php echo $data2['id'];?>,id<?php echo $data2['id'];?>)">Update</button>
                                              </div>
                                            </div>
                                          </div>
                                    </div>
