...
$companies=mysqli_query($conn,"SELECT COUNT(*) AS num FROM company WHERE status='1'");
$result_companies=mysqli_fetch_array($companies);
$individual=mysqli_query($conn,"SELECT COUNT(*) AS num FROM individual WHERE status='1'");
...
$ser_req=mysqli_query($conn,"SELECT service_requests.*,services.title,services.service_type,grouping.name as serType,individual.name as ind,individual.manage_by,individual.manage_role FROM service_requests JOIN individual ON individual.id=service_requests.individual_id INNER JOIN services ON service_requests.service_id=services.id INNER JOIN grouping ON services.service_type=grouping.id Where service_requests.status='0' AND service_requests.is_deleted='0' ORDER BY service_requests.id DESC");
...
<body >
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <a href="deleteGroup.php?id=<?php echo $data['id'];?>&table=service_requests&page=view_services_req.php"><i class="far fa-times-circle" style="color: red; font-size: 18px; margin-left: 5px;" aria-hidden="true"></i></a>
    ...
    <script type="text/javascript">
        $( document ).ready(function() {
            $.post('transaction_graph_data.php').then((result)=> {
                    var amount=result['sum'];
                    var total=0.0;
