<?php 
    ...
    $lid=$_GET['lid'];
    $letter6=mysqli_fetch_array(mysqli_query($conn,"SELECT * FROM letter_6 WHERE id='$lid'"));
    $letter6_creditors=mysqli_query($conn,"SELECT * FROM letter_six_creditors WHERE letter_id='$lid'");
    ...
    if (isset($_POST['submit'])) {
        ...
        $add_letter=mysqli_query($conn,"UPDATE letter_6 SET b_id='$bureau',datee='$date',note='$additional' WHERE id='$lid'");
        if ($add_letter) {
            $delete=mysqli_query($conn,"DELETE FROM letter_six_creditors WHERE letter_id='$lid'");
    
             if (isset($_POST['creditor'])) {
                foreach($_POST['creditor'] as $key=>$val){
                    $creditor= $_POST['creditor'][$key];

                    $allegation=mysqli_query($conn,"INSERT INTO letter_six_creditors(letter_id,creditor_id)VALUES ('$lid','$creditor')");
                    if ($allegation==false) {
                        echo $conn->error;
                    }
                }
                if ($add_letter==true && $allegation==true) {
                     header("Location: letter_preview6.php?id=$lid");    
                            ...
                            <div class="widget-header">
                                <div class="row">
                                    <div class="col-xl-12 col-md-12 col-sm-12 col-12">
                                        <h2>Additions to Credit Report</h2>
                                    </div>
                                </div>
                            </div>
                                ...
                                <form action="" method="POST" >
                                    <div class="form-row mb-4">
                                        ...
                                        <div class="form-group col-md-6">
                                            <label>Date:</label>
                                            <input type="date" name="date" value="<?php echo $letter6['datee']?>" required class="form-control"> 
                                        </div>
                                        <div class="form-group col-md-6" >
                                            <label>Bureaus:</label>
                                        ...
                                        <div class="form-group col-md-12" id="body">
                                            <?php 
                                                $count=0;
                                                while ($items=mysqli_fetch_array($letter6_creditors)) {
                                                ...
                                                <div class="row mt-2" id="<?php echo $count; ?>">
                                                    <div class="col-sm-6 ">
                                                        <select class="form-control select2"  name="creditor[]">
                                                            <option class="" value="">Select Creditors</option>
                                                            <?php
                                                                while ($data3=mysqli_fetch_array($creditors)) {
                                                                    if ($items['creditor_id']==$data3['id']) {
                                                            ?>
                                        ...
                                        <div class="form-group col-md-12 mt-4" >
                                            ...
                                            <textarea id="editor" name="additional" rows="5"><?php echo $letter6['note'] ?></textarea>
                                        </div>
                                        <hr>
                                    </div>
