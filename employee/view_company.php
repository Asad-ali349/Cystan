...
$companies=mysqli_query($conn,"SELECT company.*,individual.name as owner FROM company LEFT JOIN individual ON company.owned_by=individual.id WHERE company.managed_by='$user_id' AND company.manage_role='2' AND company.status='1' ORDER BY company.id DESC" );
