...
$companies=mysqli_query($conn,"SELECT COUNT(*) AS num FROM company WHERE status='1' AND managed_by='$user_id' AND manage_role='2'");
$result_companies=mysqli_fetch_array($companies);
$individual=mysqli_query($conn,"SELECT COUNT(*) AS num FROM individual WHERE status='1' AND manage_by='$user_id' AND manage_role='2'");
$result_individual=mysqli_fetch_array($individual);
$services=mysqli_query($conn,"SELECT COUNT(*) AS num FROM services");
$result_services=mysqli_fetch_array($services);
$services_request=mysqli_query($conn,"SELECT COUNT(*) AS num FROM service_requests");
$result_services_request=mysqli_fetch_array($services_request);
$creditors=mysqli_query($conn,"SELECT COUNT(*) AS num FROM vendors");
$result_creditors=mysqli_fetch_array($creditors);
$bureau=mysqli_query($conn,"SELECT COUNT(*) AS num FROM bureau");
$result_bureau=mysqli_fetch_array($bureau);


$ser_req=mysqli_query($conn,"SELECT service_requests.*,services.title,services.service_type,grouping.name as serType,individual.name,individual.manage_role,individual.manage_by as ind FROM service_requests JOIN individual ON individual.id=service_requests.individual_id INNER JOIN services ON service_requests.service_id=services.id INNER JOIN grouping ON services.service_type=grouping.id Where service_requests.status='0' AND individual.manage_role='2' AND individual.manage_by='$user_id' AND service_requests.is_deleted='0' ORDER BY service_requests.id DESC");
                                        ...
                                        <a href="view_people.php"><div class="widget widget-one_hybrid widget-followers">
                                            <div class="widget-heading">
                                                <div class="w-title">
                                                    ...
                                                    <div class="">
                                                        <p class="w-value"><?php echo $result_individual['num']; ?></p>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <?php if ($data['status']==2) {
                                             ?>
                                                <a href="letter/letter<?php echo $data['service_id'] ?>.php?srid=<?php echo $data['id']; ?>"><i class="far fa-file" style="color: green; font-size: 18px;" aria-hidden="true"></i></a>
                                             <?php
                                            }else{
                                            ?>
