...
if (isset($_SESSION['success']) && isset($_SESSION['error'])) {
  ...
  $ser_req=mysqli_query($conn,"SELECT service_requests.*,services.title,services.service_type,grouping.name as serType,individual.name as ind,individual.manage_role,individual.manage_by  FROM service_requests JOIN individual ON individual.id=service_requests.individual_id INNER JOIN services ON service_requests.service_id=services.id INNER JOIN grouping ON services.service_type=grouping.id Where service_requests.status='2' AND individual.manage_role='2' AND individual.manage_by='$user_id' AND service_requests.is_deleted='0' ORDER BY service_requests.id DESC");
