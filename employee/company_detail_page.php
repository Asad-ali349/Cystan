<?php
   ...
   $id=$_GET["id"];
   $companies=mysqli_query($conn,"SELECT company.id,company.loan_number, company.ein, company.customer_id,company.name,company.website,company.pin_color_id,company.total_employees,company.parent_company_id,company.group_id,company.contact,company.email,company.street,company.city,company.state,company.zip,company.country,company.review_avg,company.review_count,company.credit_score,company.anual_revenue,company.naics_id,company.latitude,company.longitude,grouping.name AS group_name,pin_color.name AS color_name,pin_color.color_code AS color_code,naics.sic_code,naics.naics_code,naics.description,naics.ins FROM company LEFT JOIN pin_color ON pin_color.id=company.pin_color_id LEFT JOIN grouping ON grouping.id=company.group_id LEFT JOIN naics ON naics.id= company.naics_id WHERE company.id='$id'" );
...
<html lang="en">
 ...
 -->
                                 ...
                                 <a href="deleteGroup.php?id=<?php echo $data2['id'];?>&table=company_docs&page=company_detail_page.php?id=<?php echo $id?>" onclick="return confirm('Are you sure?')"><i class="far fa-times-circle" style="color: red; font-size: 20px;" aria-hidden="true"></i></a>
