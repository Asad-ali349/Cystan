...
$creditor=mysqli_query($conn,"SELECT vendors.*,grouping.name as 'group' FROM vendors INNER JOIN grouping ON grouping.id=vendors.business_type ORDER BY vendors.id DESC");

$groups=mysqli_query($conn,"SELECT * FROM grouping WHERE category_id='7'");
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
                            ...
                            <table id="html5-extension" class="table table-hover non-hover" style="width:100%">
                                <thead>
                                    <tr>
                                        <th>#</th>
                                        <th>Name</th>
                                        <th class="dt-no-sorting">Business Type</th>
                                ...
                                <tbody>
                                    <?php  
                                        $count=1;
                                        while ($data2=mysqli_fetch_array($creditor)) {
                                    ?>
                                    <tr>
                                        <td><?php echo $count ?></td>
                                        <td><?php echo $data2['title']; ?></td>
                                        <td>
                                            <?php echo $data2['group'];?>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            ...
                                            <a href="deleteGroup.php?id=<?php echo $data2['id'];?>&table=vendors&page=view_inq_vendor.php" onclick="return confirm('Are you sure?')"><i class="far fa-times-circle" style="color: red; font-size: 20px;" aria-hidden="true"></i></a>
                                        </td>
                                          ...
                                          <div class="modal-dialog" role="document">
                                            <div class="modal-content">
                                              <div class="modal-header">
                                                <h5 class="modal-title"><?php echo "Update Creditor " ?></h5>
                                              ...
                                              <div class="modal-body">
                                                    ...
                                                    <select class="form-control " name="business" id="business<?php echo $data2['id'];?>">
                                                        
                                                        <?php
                                                        while ($data=mysqli_fetch_array($groups)) {
                                                            if ($data['id']==$data2['business_type']) {
                                                        ?>
                                              ...
                                              <div class="modal-footer">
                                                    <button class="btn btn-danger" onclick="updateGroup(id<?php echo $data2['id'];?>,title<?php echo $data2['id'];?>,business<?php echo $data2['id'];?>,address<?php echo $data2['id'];?>,phone<?php echo $data2['id'];?>)">Update</button>
                                              </div>
                                            </div>
                                          </div>
                                    </div>
    ...
    <script type="text/javascript">
        setTimeout(()=> {
         ...
         function updateGroup(id,title,business,address,contact){
            console.log("h")
            var bid=$(id).val();
            var bname=$(title).val();
            ...
            $.post('update_vendor.php', {id: bid,title:bname,business:business,address:address,phone:contact}).then((result)=> {
                $('.md').modal('hide');
