...
if(isset($_POST['submit'])){
    ...
    if ($manage=="supervisor") {
        ...
        $update_individual=mysqli_query($conn,"UPDATE individual SET manage_by='$sup',manage_role='1' WHERE manage_by='$sid' AND manage_role='2'");
        
        if ($update_individual==true ) {
            $block=mysqli_query($conn,"UPDATE employee SET status='0' WHERE id='$sid'");
            if ($block==true) {
            ...
            $error="Unable blocked";
        }
    }else if($manage=="employee"){
        $emp=$_POST['emp']; 
        $update_individual=mysqli_query($conn,"UPDATE individual SET manage_by='$emp',manage_role='2' WHERE manage_by='$sid' AND manage_role='2'");
        if ($update_individual==true ) {
            $block=mysqli_query($conn,"UPDATE employee SET status='0' WHERE id='$sid'");
            if ($block==true) {
                $success="blocked Successfilly";
...
$sql=mysqli_query($conn,"SELECT employee.*,supervisor.name As Supervisor FROM employee INNER JOIN supervisor ON employee.supervisor_id = supervisor.id WHERE employee.supervisor_id='$user_id' ORDER BY employee.id DESC");
?>
<!DOCTYPE html>
<html lang="en">
                            ...
                            <table id="html5-extension" class="table table-hover non-hover" style="width:100%">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="dt-no-sorting">Supervisor</th>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td><b><?php echo $data['Supervisor']; ?></b></td>
                                        <td class="justify-content-center"><span class="shadow-none badge badge-success ">
                                            <?php 
                                                $empid=$data['id'];
                                                $company_count=mysqli_query($conn,"SELECT COUNT(*) AS count from individual where manage_by='$empid' AND manage_role='2' AND status='1'");
                                        ...
                                        <td><?php 
                                            ...
                                            ?></td>
                                        <td>
                                            <a href="employee_detail_page.php?id=<?php echo $data['id'];?>"><i class="far fa-user" style="color: green; font-size: 18px;margin-right: 10px" aria-hidden="true"></i></a>
                                            <a href="update_employee.php?id=<?php echo $data['id']?>"><i class="far fa-edit" style="color: blue; font-size: 18px;margin-right: 5px" aria-hidden="true"></i></a>
                                            <?php
                                                if ($data['status']==1) {
                                            ?>
                                            ...
                                            ?>
                                                     <a href="block_unblock.php?id=<?php echo $data['id'];?>&table=employee&page=view_employee.php&status=1" onclick="return confirm('Are you sure?')"><i class="far fa-check-circle" style="color: green; font-size: 20px;" aria-hidden="true"></i></a>
                                            <?php        
                                                }
        ...
        function block(sid,mid){
            ...
            $.post("check_have_clients.php",{id:sid,role:role}).then((result)=> {
                if(result.count>0){
                    ...
                    $.get("block_unblock.php?id="+sid+"&table=employee&status=0&page=view_employee.php").then((result)=> {
