...
$comp=mysqli_query($conn,"SELECT employee.name AS emp_name,individual.*,grouping.name AS group_name FROM supervisor LEFT JOIN employee ON employee.supervisor_id=supervisor.id LEFT JOIN individual ON individual.manage_by=employee.id LEFT JOIN grouping ON grouping.id=individual.group_id WHERE individual.manage_role='2' AND supervisor.id='$user_id'  AND individual.status='1' UNION SELECT 'ME' AS emp_name,individual.*,grouping.name AS group_name FROM individual LEFT JOIN grouping ON grouping.id=individual.group_id WHERE individual.manage_role='1' AND individual.manage_by='$user_id' AND individual.status='1'");
$is_searched=0;
