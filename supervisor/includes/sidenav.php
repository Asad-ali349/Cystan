...
$compelete_sup = "SELECT Count(*) as count FROM service_requests JOIN individual ON individual.id=service_requests.individual_id INNER JOIN services ON service_requests.service_id=services.id INNER JOIN grouping ON services.service_type=grouping.id Where service_requests.status='3' AND individual.manage_role='1' AND individual.manage_by='$user_id' AND service_requests.is_deleted='0' ";
$result_complete_sup=mysqli_query($conn,$compelete_sup);

$complete_emp = " SELECT COUNT(*) as count FROM supervisor INNER JOIN employee ON employee.supervisor_id=supervisor.id INNER JOIN individual ON individual.manage_by=employee.id INNER JOIN service_requests ON service_requests.individual_id=individual.id INNER JOIN services ON service_requests.service_id=services.id INNER JOIN grouping ON services.service_type=grouping.id where individual.manage_role='2' AND employee.supervisor_id='$user_id' AND service_requests.status='3' AND service_requests.is_deleted='0';";
$result_complete_emp=mysqli_query($conn,$complete_emp);
...
if (mysqli_num_rows($result_nonapproved_sup)>0 && mysqli_num_rows($result_payment_sup)>0 && mysqli_num_rows($result_progress_sup)>0) {
    ...
    $ap_req=0;
    $datas=$result_nonapproved_sup->fetch_array();
    $ap_req+=(int)$datas['count'];
    $datas=$result_nonapproved_emp->fetch_array();
    $ap_req+=(int)$datas['count'];   
    
    $dp_req=0;
    $datas=$result_payment_sup->fetch_array();
    $dp_req+=(int)$datas['count'];
    $datas=$result_payment_emp->fetch_array();
    $dp_req+=(int)$datas['count'];

    $ir_req=0;
    $datas=$result_progress_sup->fetch_array();
    $ir_req+=(int)$datas['count'];
    $datas=$result_progress_emp->fetch_array();
    $ir_req+=(int)$datas['count'];

    $c_req=0;
    $datas=$result_complete_sup->fetch_array();
    $c_req+=(int)$datas['count'];
    $datas=$result_complete_emp->fetch_array();
    $c_req+=(int)$datas['count'];
